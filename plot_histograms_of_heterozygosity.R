...
#phals<-read.table(
#"300bp.versus.Phals_V2_repeatmasked_Nuclear_genome.fasta.rmdup.pileup.freq-first-and-second-most-abundant.csv", header=F)

#peb<-read.table("300_R1_val.versus.Pero_masked_genome_V1.fasta.pileup.freq-first-and-second-most-abundant.csv", header=F)

#emoy2<-read.table(
#"Emoy2.combined.versus.GCA_000173235.2_HyaAraEmoy2_2.0_genomic.aln.sorted.bam.pileup.freq-first-and-second-most-abundant.csv", header=F)

#infestans<-read.table(
#"ERR248813.versus.P_infestans.GCA_000142945.1_ASM14294v1_genomic.pileup.freq-first-and-second-most-abundant.csv", header=F)

#kerno<-read.table(
#"Kerno238_431.versus.GCA_000785735.2_NZFS2646v2_genomic.pileup.freq-first-and-second-most-abundant.csv",
# header=F)

#lateralis<-read.table(
#"MPF6.versus.GCA_000318465.2_MPF4_v2.0_genomic.pileup.freq-first-and-second-most-abundant.csv",
# header=F)

#ramorum<-read.table("SOD_22_12_GTAGAG_L008_R1_001_val.versus.ramorum1.fasta.pileup.freq-first-and-second-most-abundant.csv",
#header=F)

#parasitica<-read.table(
#"SRR058717.versus.S_parasitica.GCA_000151545.2_ASM15154v2_genomic.pileup.freq-first-and-second-most-abundant.csv",
# header=F)
...
#ptab<-read.table(
#"SRR2146895.versus.Ptab_968-S26_published_assembly.fasta.rename.fasta.pileup.freq-first-and-second-most-abundant.csv",
# header=F)
...
most.abundant <- genome$V3[
	      genome$V3 >= lower.lim &
	      genome$V3 <= upper.lim & 
	      !is.na(genome$V3) &
	      genome$V5 >= min.depth ]

second.most.abundant <- genome$V4[
		     genome$V4 >= lower.lim &
		      genome$V4 <= upper.lim &
		       !is.na(genome$V4) &
		       genome$V5 >= min.depth ]
